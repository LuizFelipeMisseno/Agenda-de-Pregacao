...
class EstCardWidget extends StatelessWidget {
  EstCardWidget({
    Key key,
    this.user,
    this.index,
...
class _EstudosState extends State<Estudos> {
  ...
  Future refreshPage() async {
    ...
    this.users = await UsersDatabase.instance.readEst(search);
  ...
  Future refreshUsers() async {
    ...
    this.users = await UsersDatabase.instance.readAllEst();
          ...
          return GestureDetector(
            onTap: () async {
              await Navigator.of(context).push(
                MaterialPageRoute(
                  builder: (context) => UserForm3(userId: user.id),
                ),
              );

              refreshUsers();
            },
            child: EstCardWidget(user: user, index: index),
          );
        },
      );
